module suilfg_launch::bonding_curve {
    ...
    public entry fun seed_pool_prepare<T: drop + store>(
        ...
        let p_target_u128 = (p_curve_u128 * u64::to_u128(10_000 + use_bps)) / u64::to_u128(10_000);
    ...
    public fun spot_price_u128<T: drop + store>(curve: &BondingCurve<T>): u128 {
        ...
        let s128 = u64::to_u128(s);
        (u64::to_u128(curve.m_num) * s128 * s128) / u64::to_u128(curve.m_den)
    }
    ...
    fun integrate_cost_u128(s1: u64, s2: u64, m_num: u64, m_den: u64): u128 {
        ...
        (u64::to_u128(m_num) * delta) / (u64::to_u128(3) * u64::to_u128(m_den))
    }
    ...
    fun inverse_integral_buy(s1: u64, amount_in: u64, m_num: u64, m_den: u64): u64 {
        let s1c = pow3_u128_from_u64(s1);
        let add = (u64::to_u128(3) * u64::to_u128(amount_in) * u64::to_u128(m_den)) / u64::to_u128(m_num); // floor to keep cost <= amount_in
        let x = s1c + add;
        cbrt_floor_u64(x)
    }

    fun pow3_u128_from_u64(x: u64): u128 {
        let x128 = u64::to_u128(x);
        x128 * x128 * x128
    }
    ...
    fun narrow_u128_to_u64(x: u128): u64 {
        let max64 = u64::to_u128(u64::max_value!());
        if (x > max64) { u64::max_value!() } else { u128::to_u64(x) }
    }
